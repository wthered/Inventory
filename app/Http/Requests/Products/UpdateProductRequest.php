<?php
	...
	class UpdateProductRequest extends FormRequest {
		...
		public function passedValidation(): void {
			...
			// 2. Normalize checkboxes ('on' -> true, missing/null -> false)
			$validated['track_inventory'] = $this->has('track_inventory');
			$validated['is_active'] = $this->has('is_active');

			// 3. Map form fields to database schema attributes
			...
			// 4. Ensure specifications is either valid JSON string or null
			if (empty($validated['specifications']) || $validated['specifications'] === '&hellip;') {
				$validated['specifications'] = null;
			}

			// Replace request input with cleansed/transformed payload
