<?php
	...
	class ProductDTO {
		...
		public function __construct(int $product) {
			...
			$this->inventories     = $this->product->inventories()->whereHas('location')->orderBy('warehouse_id')->paginate(25);
		...
		private function getWarehouses(array $houses): Collection {
			$warehouses = Collection::empty();
			Collection::make($houses)
				->pluck('id')
				->unique()
				->map(function (int $warehouse) use (&$warehouses) {
					$warehouses = $warehouses->push(Warehouse::query()
						->findOrFail($warehouse));
				});
			return $warehouses;
		}

		private function getInventories(array $inventories): Collection {
			$stock = Collection::empty();
			Collection::make($inventories)
				->pluck('id')
				->unique()
				->values()
				->map(function (int $inventory) use (&$stock) {
					$stock = $stock->push(Inventory::query()
						->with([
							'warehouse',
							'location'
						])
		...
		private function getLocations(array $locations): Collection {
			$productLocations = Collection::empty();
			Collection::make($locations)
				->unique()
				->each(function ($location) use (&$productLocations) {
					$productLocations = $productLocations->push(WarehouseLocation::query()
						->with('warehouses')
