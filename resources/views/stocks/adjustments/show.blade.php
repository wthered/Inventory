...
@section('content')
	<div class="main-container">
		...
		<div class="back-action" style="margin-bottom: 1rem;">
			<a href="{{ route('inventory.adjustments.index') }}" style="text-decoration: none; color: #64748b; font-weight: 500; font-size: 0.9rem; display: inline-flex; align-items: center; gap: 0.25rem;">
		...
		<div class="page-header-container">
			...
			<div class="header-actions" style="display: flex; gap: 0.75rem; align-items: center;">
				...
				<div class="split-btn-group" style="position: relative; display: inline-flex; vertical-align: middle;">
					...
					<form id="status-split-form" action="{{ route('inventory.adjustments.approve', $adjustment->id) }}" method="POST" style="margin: 0; display: inline-flex;">
						@csrf
						@method('PATCH')

						{{-- Διασφάλιση ότι παίρνουμε το value του Enum --}}
						<input type="hidden" name="status" id="selected-status-value" value="{{ $adjustment->status->value }}">

						<button type="submit" id="main-action-btn" class="btn" style="background-color: #0f172a; color: white; border: none; padding: 0.6rem 1.2rem; border-top-right-radius: 0; border-bottom-right-radius: 0; font-weight: 600; cursor: pointer; border-right: 1px solid #334155;" onclick="return confirm('Επιβεβαίωση αλλαγής κατάστασης;');">
							💾 Αποθήκευση Κατάστασης
						</button>
					</form>

					{{-- Το Βέλος (Trigger για το Dropdown) --}}
					<button type="button" id="dropdown-toggle-btn" class="btn" style="background-color: #0f172a; color: white; border: none; padding: 0.6rem 0.8rem; border-top-left-radius: 0; border-bottom-left-radius: 0; cursor: pointer;" onclick="toggleSplitDropdown()">
						▼
					</button>

					{{-- Το Μενού με τις Επιλογές των Status --}}
					<ul id="split-dropdown-menu" style="position: absolute; top: 100%; right: 0; z-index: 1000; display: none; min-width: 220px; padding: 0.5rem 0; margin: 0.25rem 0 0; font-size: 0.9rem; color: #1e293b; text-align: left; list-style: none; background-color: #ffffff; background-clip: padding-box; border: 1px solid #cbd5e1; border-radius: 6px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06);">
				...
				@if($canBeEdited)
					<a href="{{ route('inventory.adjustments.edit', $adjustment->id) }}" class="btn btn-primary" style="padding: 0.6rem 1.2rem; border-radius: 4px;">
						✏️ Επεξεργασία
					</a>
				@endif
			</div>
		...
		<div class="details-grid">
            ...
            <div class="info-box" style="border-left: 4px solid {{ $statusBorderColor }}; background-color: {{ $statusColor }};">
		...
		@if(!empty($adjustment->notes))
			<div class="notes-box">
				<div class="info-label" style="color: #856404; margin-bottom: 0.5rem;">Σημειώσεις / Παρατηρήσεις Εγγράφου</div>
		...
		<div class="table-container" style="margin-top: 2rem;">
			...
			<table class="data-table">
				...
				@empty
					<tr>
						<td colspan="6" class="empty-table-state">Δεν βρέθηκαν γραμμές προϊόντων για αυτή την προσαρμογή.</td>
					</tr>
				@endforelse
				</tbody>
			</table>
