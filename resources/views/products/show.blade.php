	...
	<div class="product-container">
		...
		<div class="product-header">
			<div class="breadcrumb">
				<a href="{{ route('inventory.products.index') }}"><i class="fas fa-arrow-left"></i> Back to Products</a>
			</div>
			<div class="product-actions">
				<a href="{{route('inventory.products.edit', ['product' => $product->id])}}" class="btn-icon"
				   title="Edit Product">
			...
			<div class="product-images">
				...
				<div class="image-thumbnails">
					@foreach($product->images as $image)
						<img src="{{ $image->image_location }}" alt="Thumbnail" class="thumbnail {{ $image->is_default ? 'active' : '' }}">
					@endforeach
				</div>
			</div>
			...
			<div class="product-details">
				...
				<div class="product-pricing">
					...
					<div class="price-group">
						...
						<span
							class="price price-profit">&euro;{{ $profit['absolute'] }} ({{ $profit['relative'] }}%)</span>
					</div>
				</div>
			...
			<div class="inventory-cards">
				<div class="inventory-card">
					...
					<div class="card-content">
						...
						<span class="card-value">{{ number_format($stock['total'], 0, '', '.') }} pcs</span>
					</div>
				</div>
				<div class="inventory-card">
				...
				<table class="data-table">
					...
					@foreach($inventories as $inventory)
						<tr>
							...
							<td>
								<span class="status-badge status-{{ $status[$inventory->warehouse_id][$inventory->location_id]['tier_label'] }}" title="{{ $status[$inventory->warehouse_id][$inventory->location_id]['suggested_action'] }}">
									{{ $status[$inventory->warehouse_id][$inventory->location_id]['tier_label'] }}
								</span>
							</td>
							<td class="actions">
								<button class="btn-sm btn-primary transfer-btn" title="Transfer"
								...
								<button class="btn-sm btn-secondary btn-adjust" title="Adjust"
										data-product="{{ $product->id }}"
										data-warehouse="{{ $inventory->warehouse_id }}"
										data-location="{{ $inventory->location_id }}"
					...
					<form id="transferForm">
						...
						<div class="form-group">
							...
							<div class="quantity-controls">
								...
								<div class="available-quantity">Available: <div id="availableQuantity">0</div></div>
							</div>
						</div>

						<div class="form-group">
							<label for="transferNotes">Transfer Notes (Optional)</label>
							<textarea id="transferNotes" class="form-control" rows="3" placeholder="Reason for transfer, special handling instructions..."></textarea>
						</div>

						<div class="form-actions">
							<button type="button" class="btn btn-secondary" id="cancelTransfer">Cancel</button>
							<button type="submit" class="btn btn-primary" id="confirmTransfer">Transfer Products</button>
						</div>
					</form>
				</div>
			</div>
		</div>
		<!-- Transaction Modal Finish -->
				...
				<form id="adjustmentForm" action="#" method="POST">
					...
					<div class="modal-body">
						...
						<div class="form-group">
							...
							<div class="quantity-controls">
								<input type="number" class="form-control" id="adjustCurrentQty" readonly style="background-color: #f8f9fa;">
						...
						<div class="form-row">
							<div class="form-group">
								...
								<div class="adjustment-type-selector" style="margin-top: 0.25rem;">
									<div class="type-radio-group">
										...
										<input type="radio" id="typeDecrease" name="type" value="decrease" class="type-radio">
							...
							<div class="form-group">
								...
								<div class="quantity-controls">
									<input type="number" name="quantity" id="adjustQuantity" min="1" max="{{ !empty($product->max_stock_level) ? $product->max_stock_level - $product->current_stock : $product->current_stock }}" value="1" required class="form-control">
								</div>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<textarea name="notes" id="adjustNotes" rows="3" class="form-control" placeholder="Additional details about this adjustment..."></textarea>
						</div>

						<!-- Preview Section -->
						<div class="location-section" style="background-color: #f8f9fa; border-color: #dee2e6;">
							<h3>Adjustment Preview</h3>
							<div class="preview-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
								<div class="preview-item">
									<div style="font-size: 12px; color: #6c757d; margin-bottom: 4px;">Current</div>
									<div id="previewCurrent" style="font-size: 24px; font-weight: 600; color: #495057;">0</div>
								</div>
								<div class="preview-item">
									<div style="font-size: 12px; color: #6c757d; margin-bottom: 4px;">Change</div>
									<div id="previewChange" style="font-size: 24px; font-weight: 600; color: #28a745;">+0</div>
								</div>
								<div class="preview-item" style="grid-column: span 2;">
									<div style="font-size: 12px; color: #6c757d; margin-bottom: 4px;">New Quantity</div>
									<div id="previewNew" style="font-size: 32px; font-weight: 700; color: #667eea;">0</div>
								</div>
							</div>
						</div>
					</div>
					...
					<div class="form-actions">
						<button type="button" class="btn" style="background-color: #6c757d; color: white;" id="adjustCancelBtn">
							Cancel
						</button>
						<button type="submit" class="btn adjustment-btn"><i class="fas fa-check"></i> Apply Adjustment</button>
					</div>
				</form>
			</div>
		</div>
		<!-- Adjustment Modal Finish -->
			...
			<div class="table-container">
				<table class="data-table">
					...
					@foreach($product->transactions as $transaction)
						<tr>
							{{-- Χρήση format για πιο όμορφη ημερομηνία --}}
							<td>{{ $transaction->updated_at->format('d/m/Y H:i') }}</td>

							{{-- Δυναμικό Type Badge --}}
							<td>
								<span class="transaction-type type-{{ $transaction->type->value ?? $transaction->type }}">{{ $transaction->type->value ?? $transaction->type }}</span>
							</td>

							<td>
								<div class="reference-wrapper">
									<span class="ref-pill {{ $transaction->reference_display['class'] }}"><i class="fas {{ $transaction->reference_display['icon'] }}"></i> {{ $transaction->reference_display['label'] }}</span>
								</div>
							</td>

							{{-- Warehouse ή Transfer Note --}}
							<td>
								<div class="warehouse-info">
							...
							{{-- Δυναμικό χρώμα στο Quantity: Πράσινο για IN, Κόκκινο για OUT --}}
