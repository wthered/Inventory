						...
						<div class="form-group">
							...
							<select id="category_id" name="parent_category" required>
								...
								@foreach($categories as $category)
									<option value="{{ $category->id }}" @if(old('parent_category', $category->id) == $parent_category->id) selected @endif>{{ $category->name }}</option>
								@endforeach
							</select>
					...
					<div class="form-grid">
						<div class="group-box">
							<div class="form-group checkbox-group">
								...
								<input type="checkbox" id="track_inventory" name="track_inventory" @checked(old('is_active', $product->track_inventory))>
							</div>
						...
						<div class="group-box">
							<div class="form-group checkbox-group">
								...
								<input type="checkbox" id="is_active_toggle" name="is_active" @checked(old('is_active', $product->is_active))>
							</div>
...
@section('scripts')
	<script>
		...
		function refreshPreview(imageUrl) {
			fetch('{{ route('inventory.products.image.remove', ['product' => $product->id])}}', {
				method: 'DELETE',
				headers: {
		...
		document.getElementById('image_upload').addEventListener('change', function (event) {
			...
			console.log('You selected:', files.length, 'file(s)');
			...
			fetch('{{ route('inventory.products.image.upload', ['product' => $product->id]) }}', {
				...
				if (data.success) {
					...
					// If your delete buttons are dynamically added
					// reattach event listeners for them here
					attachDeleteListeners();
				} else {
